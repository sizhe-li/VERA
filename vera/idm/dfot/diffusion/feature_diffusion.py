...
from ..backbones import DiT3D, DiT3DPose, DiT3DSpatial, Unet3D, UViT3D, UViT3DPose
...
class FeatureDiffusion(nn.Module):
    ...
    def forward(
        ...
        # flow_pred = rearrange(pred, "B T C H W -> B H W T C")  # to (B,H,W,T,2)
        # flow_gt = rearrange(target, "B T C H W -> B H W T C")  # to (B,H,W,T,2)
        # assert flow_pred.shape[-1] == 2
        # flow_gt_scaled, w_map, m_mov = prepare_flow_supervision(
        #     flow_pxps=flow_gt, tau=1e-3, bg_weight=0.1
        # )
        # # Charbonnier loss with spatial weights
        # eps = 1e-3
        # diff = flow_pred - flow_gt_scaled
        # charbonnier = torch.sqrt(diff.pow(2) + eps**2)
        # loss_map = charbonnier.mean(dim=1)  # average over 2 comps
        # loss = (loss_map * w_map).sum() / (w_map.sum() + 1e-6)
