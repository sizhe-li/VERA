...
from vera.idm.common.metrics.video import (
    SharedVideoMetricModelRegistry,
    VideoMetric,
)
from vera.idm.registry import register_algorithm
from vera.idm.vae import ImageVAE, VideoVAE
from vera.utils.distributed_utils import (
    is_rank_zero,
    rank_zero_print,
)
from vera.utils.logging_utils import log_video
...
@register_algorithm(
    "dfot_video_action_latent",
    cfg_cls=None,
)
class DFoTVideoActionLatent(BasePytorchAlgo):
    """
    ...
    def on_after_batch_transfer(
        ...
        if self.is_latent_diffusion:
            xs = (
                self._encode(batch[videos_key])
                if self.is_latent_online
                else batch["latents"]
            )
            if videos_key in batch:
                gt_videos = batch[videos_key]
        else:
            xs = batch[videos_key]
        ...
        if self.external_cond_dim > 0:
            conditions = batch.get(conditions_key, None)
