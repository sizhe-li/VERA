...
@register_algorithm("dfot_motion_policy", cfg_cls=None)
class DFoTMotionPolicy(BasePytorchAlgo):
    """
    ...
    def on_after_batch_transfer(
        ...
        # xs/conditions shape is (batch, time, C, H, W); time T is preserved
        ...
        conditions = rgb
        # zero out conditions after context length
        conditions[:, self.n_context_frames :] = 0.0
        ...
        return xs, xs_mask, conditions, masks, rgb
    ...
    def training_step(self, batch, batch_idx, namespace="training") -> STEP_OUTPUT:
        """Training step"""
        xs, xs_mask, conditions, masks, *_ = batch
        ...
        # loss = self._reweight_loss(loss, xs_mask * masks[:, :, None, None, None])
        # loss = self._reweight_loss(loss, xs_mask * masks[:, :, None, None, None])
        ...
        if batch_idx % self.cfg.logging.loss_freq == 0:
            ...
            # log the min max of xs, conditions
            # --- Log sanity checks ---
    ...
    def _eval_denoising(self, batch, batch_idx, namespace="training") -> None:
        ...
        xs, xs_mask, conditions, masks, gt_videos = batch
        ...
        # update batch variable after applying masks
        batch = (xs, xs_mask, conditions, masks, gt_videos)
        ...
        self._log_rgb_condition(gt_videos, num_videos_to_log, self.global_step)

        # Log motion tracks
        self._log_motion_tracks(
            recon_xs, gt_xs, gt_videos, xs_mask, num_videos_to_log, self.global_step
        )
    ...
    def _eval_conditional_generation(self, batch, batch_idx, namespace="validation"):
        ...
        """Evaluate the denoising performance during training."""
        gt_xs, xs_mask, conditions, masks, gt_videos = batch
        ...
        # update batch variable after applying masks
        batch = (gt_xs, xs_mask, conditions, masks, gt_videos)

        # gather data from all gpus
        # gt_xs, conditions, xs_mask, gt_videos = self.gather_data(
        #     (gt_xs, conditions, xs_mask, gt_videos)
        # )
        ...
        self._log_motion_tracks(
            ...
            namespace="conditional generation",
        )
    ...
    def _sample_all_videos(
        ...
        xs, xs_mask, conditions, *_, gt_videos = batch
