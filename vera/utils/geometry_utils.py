...
ResizeRounding = Literal["floor", "round", "ceil"]


def uniform_resize_center_crop_params(
    src_h: int,
    src_w: int,
    target_h: int,
    target_w: int,
    *,
    ...
    """
    Return the isotropic scale and centered crop parameters used by
    `uniform_resize_center_crop`.
    """
    if src_h == target_h and src_w == target_w:
        return 1.0, src_h, src_w, 0, 0
    ...
    if new_h < target_h or new_w < target_w:
        raise RuntimeError(
            f"Resized shape {(new_h, new_w)} does not cover target {(target_h, target_w)} "
            f"for source {(src_h, src_w)} with scale={scale:.8f} and rounding={rounding}."
        )

    y_start = (new_h - target_h) // 2
    x_start = (new_w - target_w) // 2
    return scale, new_h, new_w, y_start, x_start
...
def uniform_resize_center_crop(
    ...
    *,
    ...
    """
    Uniformly scale [N,C,H,W] until it covers the target size, then center crop.
    Returns the resized tensor and the isotropic scale factor that was applied.
    """
    if tensor.ndim != 4:
        raise ValueError(f"Expected tensor shape [N,C,H,W], got {tuple(tensor.shape)}")

    src_h, src_w = tensor.shape[-2], tensor.shape[-1]
    if src_h == target_h and src_w == target_w:
        return tensor, 1.0

    scale, new_h, new_w, y_start, x_start = uniform_resize_center_crop_params(
        src_h, src_w, target_h, target_w, rounding=rounding
    )

    tensor = F.interpolate(
        tensor, size=(new_h, new_w), mode="bilinear", align_corners=False
    )
    tensor = tensor[:, :, y_start : y_start + target_h, x_start : x_start + target_w]
    return tensor.contiguous(), scale


def resize_flow_uniform_center_crop(
    flow: torch.Tensor,
    target_h: int,
    target_w: int,
    *,
    ...
    """
    Resize flow [N,2,H,W] with the same uniform-resize-then-center-crop policy used
    for RGB, and rescale the flow vectors by the isotropic resize factor.
    """
    if flow.ndim != 4 or flow.shape[1] != 2:
        raise ValueError(f"Expected flow shape [N,2,H,W], got {tuple(flow.shape)}")

    flow, scale = uniform_resize_center_crop(
        flow, target_h, target_w, rounding=rounding
    )
    flow = flow.clone()
    flow[:, 0] *= scale
    flow[:, 1] *= scale
